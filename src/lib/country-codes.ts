...
// Maps ISO 3166-1 numeric codes (used as the `id` field on world-atlas /
// TopoJSON country geometries) to ISO 3166-1 alpha-3 codes (used as the
// `code` field in our database). Matching by this numeric id is far more
// reliable than matching by country name, since map data and our database
// sometimes spell country names differently (e.g. "United States of
// America" on the map vs "United States" in the database).
...
export function numericIdToAlpha3(numericId: string | number | undefined): string | undefined {
  if (numericId == null) return undefined;
  // The map data may give ids as "840" or "40" (no leading zeros) depending
  // on the source, so try both the raw value and a zero-padded version.
  const raw = String(numericId);
  const padded = raw.padStart(3, "0");
  return NUMERIC_TO_ALPHA3[raw] ?? NUMERIC_TO_ALPHA3[padded];
}
